...
/// FrameScene, holding its width, height and private flat array(vec) of pixel data,
...
impl FrameScene {
    /// Generates new FrameScene struct with specified width and height.
    /// Pixel data format is assumed to be rgb8.
    pub fn new(width: u32, height: u32) -> FrameScene {
    ...
    /// Checking if coordinate is in FrameScene bounds.
    ...
    /// Sets FrameScene pixel to a color at specifed coordinate.
    /// 
    ...
    pub fn set_pixel(&mut self, p: Vector2i, color: Color) {
        ...
        // @TRASH forcing (0, 0) to be in the bottom left here by inverting coord.y.
    ...
    pub fn draw_triangle(&mut self, a: Vector3f, b: Vector3f, c: Vector3f, color: Color) {
            ...
            for j in bbox.ll.y..=bbox.ur.y {
                ...
                if barycentric_coord.x < 0.0 || barycentric_coord.y < 0.0 || barycentric_coord.z < 0.0 {
                    ...
                    if z_to_screen > self.z_buffer[index] {
                        ...
                        self.set_pixel(coord_point, color);
                    }
                }
            }
        }
